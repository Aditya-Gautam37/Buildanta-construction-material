  ...
  const navigation = <>
    {groups.map((group) => <section key={group.label} className="space-y-1">
      ...
      {group.items.map((item) => {
        ...
        return <Link key={item.href} href={item.href} onClick={() => setMobileOpen(false)} className={`group flex min-h-11 items-center gap-3 rounded-xl px-3 text-sm font-semibold transition ${active ? "bg-[#12344a] text-white shadow-sm" : "text-slate-600 hover:bg-slate-100 hover:text-slate-950"}`}>
  ...
  return <div className="min-h-screen bg-[#f4f7fa]">
    <aside className="fixed inset-y-0 left-0 z-50 hidden w-64 flex-col border-r border-slate-200 bg-white lg:flex">
      <Link href="/dashboard" className="flex h-[76px] items-center gap-3 border-b border-slate-100 px-5">
        <span className="grid size-11 place-items-center rounded-2xl bg-[#12344a] text-lg font-black text-white shadow-lg shadow-slate-900/10">B</span>
    ...
    <div className="min-w-0 lg:pl-64">
      <header className="sticky top-0 z-40 flex h-16 items-center justify-between gap-4 border-b border-slate-200/80 bg-white/90 px-4 backdrop-blur-xl sm:px-6 lg:px-8">
        ...
        <div className="flex items-center gap-2"><span className="hidden rounded-full bg-emerald-50 px-3 py-1.5 text-[11px] font-bold text-emerald-700 sm:inline-flex"><i className="mr-2 mt-1 size-1.5 rounded-full bg-emerald-500" />API connected</span>{userEmail ? <form action={signOutAction}><button type="submit" className="inline-flex h-10 items-center gap-2 whitespace-nowrap rounded-xl bg-slate-950 px-3.5 text-xs font-bold text-white transition hover:bg-[#12344a]"><LogOut className="size-4" /><span className="hidden sm:inline">Log out</span></button></form> : <Link href="/login" className="rounded-xl bg-slate-950 px-4 py-2.5 text-xs font-bold text-white">Login</Link>}</div>
      </header>
      {children}
    </div>
  </div>
