...
export function CalculatorWizard({ calculator }: { calculator: PublicCalculator }) {
  ...
  function downloadBoq() {
    ...
    const rows = [
      ...
      ...estimate.items.map((item) => [item.group, item.description, item.product?.name || "Optional inventory mapping pending", item.rawQuantity, item.formulaUnitCode, item.wastageQuantity, item.purchaseQuantity, item.unitCode, item.unitPrice || "Not included", item.gstPercent || "", item.lineTotal || "", item.availabilityLabel]),
    ...
    <div className={`${styles.layout} ${estimate && isBuildingPlan ? styles.layoutWithBoq : ""}`}>
      <section className={`${styles.panel} ${styles.inputPanel}`}>
        ...
        <form onSubmit={calculate} className={styles.formGrid}>
          ...
          <Select label={isBuildingPlan ? "Preferred material tier (for quotation)" : "Material preference (for quotation)"} name="qualityTier" options={[["STANDARD", "Standard"], ["ECONOMY", "Economy"], ["PREMIUM", "Premium"]]} />
      ...
      <section className={`${styles.panel} ${styles.resultPanel} ${estimate && isBuildingPlan ? styles.boqPanel : ""} ${calculating ? styles.resultLoading : ""}`} id="calculator-results" aria-busy={calculating}>
        <div className={styles.resultHeader}>
          <div><span className={styles.boqKicker}>{isBuildingPlan ? "PRELIMINARY BILL OF QUANTITIES" : "MATERIAL ESTIMATE"}</span><h2>{isBuildingPlan ? "Project BOQ schedule" : "Your material estimate"}</h2><p className={styles.muted}>{estimate ? "A versioned quantity schedule saved in Buildanta Inventory; catalogue mappings are optional." : "Complete the project brief to generate material-wise quantities. Products and pricing can be assigned later."}</p></div>
        ...
        {estimate ? <>
          {isBuildingPlan && <>
            ...
            <div className={styles.costOverview}>
              <div className={styles.costHero}><span>{pricedLineCount ? "Indicative mapped-material total" : "Quantity estimate ready"}</span><strong>{pricedLineCount ? money(estimate.indicativeTotal) : `${estimate.items.length} material lines`}</strong><small>{pricedLineCount ? "Only fully mapped prices contribute to commercial totals. Final pricing and freight are confirmed in quotation." : "No product or price data is required. Inventory staff can map products and prepare pricing later."}</small></div>
              <div className={styles.costMetrics}><div><span>Materials</span><strong>{estimate.subtotal == null ? "Not priced" : money(estimate.subtotal)}</strong></div><div><span>GST snapshot</span><strong>{estimate.gstTotal == null ? "Not priced" : money(estimate.gstTotal)}</strong></div><div><span>Inventory mapped</span><strong>{pricedLineCount} of {estimate.items.length}</strong></div></div>
            </div>
            ...
            {visibleGroups.map(([group, items]) => {
              ...
              return <section className={styles.resultGroup} key={group}>
                ...
                {items.map((item) => <article className={styles.boqRow} role="row" key={item.id}>
                  ...
                  <div className={styles.materialCell}>{item.product?.image ? <img src={item.product.image.src} alt="" /> : <i>{item.description.slice(0, 1)}</i>}<div><strong>{item.product?.name || item.description}</strong><small>{item.product ? `${item.product.brand} / ${item.variant?.sku || "Mapped product"}` : "Generic calculated material / inventory mapping optional"}</small>{item.product && <em><span aria-hidden="true" />{item.availabilityLabel}{item.leadTimeLabel ? ` - ${item.leadTimeLabel}` : ""}</em>}</div></div>
          ...
          <div className={styles.quote} id="quote-request">
            <div className={styles.quoteIntro}><span className={styles.quoteIcon} aria-hidden="true">BQ</span><div><span className={styles.panelEyebrow}>Next step</span><h3>Turn this BOQ into a commercial quotation</h3><p>Send all {estimate.items.length} calculated material lines to Buildanta Inventory. Staff can select products, prices and fulfilment details while preparing the quotation.</p></div></div>
