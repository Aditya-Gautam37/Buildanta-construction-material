...
export async function acceptQuotation(form:FormData){let target="/account?notice=Quotation%20accepted.";try{await customerPortalMutation(`quotations/${String(form.get("quotationId"))}/accept`)}catch(error){target=`/account?error=${encodeURIComponent(message(error))}`}revalidatePath("/account");redirect(target)}
