...
const button = "h-11 rounded-xl bg-[#0b2b3f] px-5 text-sm font-semibold text-white transition hover:bg-emerald-700 disabled:cursor-not-allowed disabled:bg-slate-300"
  ...
  return <main className="mx-auto w-full max-w-[1500px] flex-1 space-y-8 px-4 py-8 sm:px-6 lg:px-8">
    <section className="overflow-hidden rounded-3xl bg-[#0b2b3f] p-6 text-white shadow-xl sm:p-8">
      ...
      <div className="mt-3 grid gap-6 xl:grid-cols-[1fr_auto] xl:items-end">
        <div><h1 className="text-3xl font-bold tracking-tight sm:text-4xl">Calculator control centre</h1><p className="mt-2 max-w-3xl text-sm leading-6 text-slate-300">Manage validated formula versions, connect outputs to catalogue variants, publish safely, and trace every customer estimate into its quotation.</p></div>
    ...
    {canEdit && <section className="space-y-5">
      ...
      {overview.definitions.map((definition) => {
        ...
        return <article key={definition.id} className={panel}>
          ...
          {draft && <section className="mt-5 rounded-2xl border border-orange-200 bg-orange-50/50 p-4"><div className="flex flex-wrap items-center justify-between gap-3"><div><h4 className="font-bold">Prepare draft version {draft.version}</h4><p className="text-sm text-slate-600">Map every formula output before an administrator publishes it.</p></div><Status value={draft.status}/></div>
