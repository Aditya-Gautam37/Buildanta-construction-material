...
var client_references_default = {
	"06617d537330": async () => {
		const m = await import("./site-chrome-BPyzLR7b.mjs");
		return {
			get "Footer"() {
				return m["Footer"];
			},
	...
	"0d5138b86f6e": async () => {
		const m = await import("./product-detail-client-ZTrF4Zos.mjs");
		return { get "ProductDetailClient"() {
			return m["ProductDetailClient"];
		} };
	},
	"15c18cfaeeff": async () => {
		const m = await import("./layout-segment-context-CXlhM4Hp.mjs");
		return { get "LayoutSegmentProvider"() {
			return m["LayoutSegmentProvider"];
		} };
	},
	"2267e6aaddbb": async () => {
		const m = await import("./calculator-wizard-D48ZdnUF.mjs");
		return { get "CalculatorWizard"() {
			return m["CalculatorWizard"];
		} };
	},
	"251bfebdf721": async () => {
		const m = await import("./supplier-form-DhBj7q47.mjs");
		return { get "SupplierForm"() {
			return m["SupplierForm"];
		} };
	},
	"385723b0c19e": async () => {
		const m = await import("./quote-form-ClM_NkGH.mjs");
		return { get "QuoteForm"() {
			return m["QuoteForm"];
		} };
	},
	"47c6044e280a": async () => {
		const m = await import("./serviceability-checker-CKsZWYM2.mjs");
		return { get "ServiceabilityChecker"() {
			return m["ServiceabilityChecker"];
		} };
	},
	"593f344dc510": async () => {
		const m = await import("./error-boundary-BgC9EvY0.mjs");
		return {
			get "ErrorBoundary"() {
	...
	"5d017b3bfd6a": async () => {
		const m = await import("./customer-logout-button-CJL-eDu8.mjs");
		return { get "CustomerLogoutButton"() {
			return m["CustomerLogoutButton"];
		} };
	},
	"6178223e1909": async () => {
		const m = await import("./page-CWvAiKW1.mjs");
		return { get "default"() {
			return m["default"];
		} };
	},
	"893c93e3d3a0": async () => {
		const m = await import("./hero-slider-Dci043fV.mjs");
		return { get "HeroSlider"() {
			return m["HeroSlider"];
		} };
	},
	"8c0f216c4604": async () => {
	...
	"a1cb92d99abb": async () => {
		const m = await import("./brand-mark-1NQSgsfM.mjs");
		return { get "BrandMark"() {
			return m["BrandMark"];
		} };
	},
	"ea2b1fde91c1": async () => {
		const m = await import("./submit-button-CTW5MkOX.mjs");
		return { get "SubmitButton"() {
			return m["SubmitButton"];
		} };
	},
	"fbbd38950b66": async () => {
		const m = await import("./product-browser-C8w1Fze0.mjs");
		return {
			get "ProductBrowser"() {
				return m["ProductBrowser"];
			},
			get "ProductCard"() {
				return m["ProductCard"];
			}
		};
	},
	"ff0ff5bd658b": async () => {
		const m = await import("./customer-auth-form-CK90bhRD.mjs");
		return { get "CustomerAuthForm"() {
			return m["CustomerAuthForm"];
		} };
	}
};
...
//#endregion
export { VINEXT_PARAMS_HEADER as _, useRouter as a, createRscRequestUrl as c, toBrowserNavigationHref as d, app_ssr_entry_default as default, toSameOriginAppPath as f, VINEXT_MOUNTED_SLOTS_HEADER as g, require_react as h, handleSsr, usePathname as i, notifyAppRouterTransitionStart as l, AppElementsWire as m, require_jsx_runtime as n, ReadonlyURLSearchParams as o, assertSafeNavigationUrl as p, getLayoutSegmentContext as r, createRscRequestHeaders as s, require_react_dom as t, isHashOnlyBrowserUrlChange as u, stripBasePath as v, __toESM as y };
