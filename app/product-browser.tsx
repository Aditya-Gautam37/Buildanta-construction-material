...
export function ProductCard({ product }: { product: StoreProduct }) {
  return <article className="product-card">
    <a className={`product-visual ${product.image ? "has-image" : ""}`} href={`/products/${product.slug}`}><span className="product-brand">{product.brand}</span>{product.image ? <img src={product.image} alt={product.imageAlt} loading="lazy" /> : <b>{product.category.split(" ")[0]}</b>}<i>Available for enquiry</i></a>
