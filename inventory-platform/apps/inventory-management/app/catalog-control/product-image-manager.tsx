...
export default function ProductImageManager({ productId, productName, initialImages }: { productId: string; productName: string; initialImages: ImageRow[] }) {
  ...
  return <section>
    ...
    <div className="mt-4 flex flex-col gap-3 rounded-2xl bg-[#f4f7fa] p-3 sm:flex-row sm:items-center sm:justify-between"><p className="text-[11px] leading-5 text-slate-500"><strong className="text-slate-700">{images.length} image{images.length === 1 ? "" : "s"}</strong> prepared. Save after uploading, removing or changing the primary image.</p><button type="button" onClick={() => void save()} disabled={busy} className="min-h-10 rounded-xl bg-[#123a5e] px-4 text-xs font-bold text-white transition hover:bg-emerald-700 disabled:opacity-50">{busy ? "Saving gallery..." : "Save gallery"}</button></div>
  </section>
}
