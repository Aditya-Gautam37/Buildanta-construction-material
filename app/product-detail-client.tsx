...
import { availabilityStatusLabel, type StoreProduct } from "./live-catalog";
...
export function ProductDetailClient({ product }: { product: StoreProduct }) {
  ...
  return <section className="product-detail">
    <div className="product-gallery"><div className={`product-detail-visual ${images.length ? "has-image" : ""}`}><span>{product.brand}</span>{images[imageIndex] ? <img src={images[imageIndex].src} alt={images[imageIndex].alt} decoding="async" fetchPriority="high" /> : <b>{product.category.split(" ")[0]}</b>}</div>{images.length > 1 && <div className="product-thumbnails" aria-label="Product images">{images.map((image, index) => <button key={`${image.src}-${index}`} className={index === imageIndex ? "active" : ""} onClick={() => setImageIndex(index)} aria-label={`View image ${index + 1}`}><img src={image.src} alt="" loading="lazy" decoding="async" /></button>)}</div>}</div>
    <div className="product-detail-copy"><p>{product.brand} · {product.category}</p><h1>{product.name}</h1><p className="detail-description">{product.description}</p>{product.variants.length > 0 && <label className="variant-selector"><span>Choose variant</span><select value={variantId} onChange={(event) => setVariantId(event.target.value)}>{product.variants.map((item) => <option key={item.id} value={item.id}>{item.label} · {item.sku}</option>)}</select></label>}<div className="price-box"><span>{price > 0 ? "Indicative demonstration price" : "Pricing"}</span><strong>{price > 0 ? `₹${price.toLocaleString("en-IN")}` : "Request latest price"}</strong>{price > 0 && <small> / {unit}</small>}{product.bulkPrice != null && <em>Indicative bulk price: ₹{product.bulkPrice.toLocaleString("en-IN")} / {product.unit}</em>}</div><p className={`availability ${selectedAvailability === "LOW_STOCK" || selectedAvailability === "OUT_OF_STOCK" ? "limited" : ""}`}><i /> {availability}</p><ServiceabilityChecker productId={product.id}/><dl className="product-facts"><div><dt>SKU</dt><dd>{variant?.sku || product.sku}</dd></div><div><dt>Minimum order</dt><dd>{product.minimumOrderQuantity} {product.unit}</dd></div><div><dt>GST</dt><dd>{product.gstPercent == null ? "Confirmed in quotation" : `${product.gstPercent}%`}</dd></div><div><dt>Fulfilment</dt><dd>Confirmed with quotation</dd></div></dl><p className="detail-description">{product.deliveryInfo || "Delivery confirmed after PIN-code review."} GST and transportation are confirmed in the quotation.</p>{variant && variant.purchaseMode !== "QUOTE_ONLY" && <AddToCartButton variantId={variant.id} minimumOrderQuantity={variant.minimumOrderQuantity} />}<KnowYourMaterialTrigger productId={product.id} productName={product.name} /><a className="button orange wide" href={`/bulk-quotes?product=${encodeURIComponent(product.name)}`}>Request latest price <span>→</span></a><small>Final pricing depends on quantity, delivery location and verified fulfilment availability.</small></div>
  </section>;
}
